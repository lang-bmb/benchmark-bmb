#!/bin/bash
# BMB Benchmark Runner
# Builds and runs all benchmarks, collecting timing data
# Compares BMB vs C (Clang) vs Rust

BMB="D:/data/lang-bmb/target/x86_64-pc-windows-gnu/release/bmb.exe"
RESULTS_DIR="benchmark_results"
mkdir -p "$RESULTS_DIR"

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
NC='\033[0m' # No Color

echo "=============================================="
echo "BMB Benchmark Suite v0.60.263"
echo "Baseline: Clang -O3, Rust --release (LLVM)"
echo "=============================================="
echo ""

# Function to time a command (returns milliseconds)
time_cmd() {
    local start=$(date +%s%N)
    "$@" > /dev/null 2>&1
    local end=$(date +%s%N)
    echo $(( (end - start) / 1000000 ))
}

# Function to run benchmark multiple times and get median
run_benchmark() {
    local exe=$1
    local runs=5
    local times=()

    for ((i=1; i<=runs; i++)); do
        local t=$(time_cmd "$exe")
        times+=($t)
    done

    # Sort and get median
    IFS=$'\n' sorted=($(sort -n <<<"${times[*]}")); unset IFS
    local median=${sorted[$((runs/2))]}
    echo $median
}

# Build and benchmark function
benchmark() {
    local name=$1
    local category=$2
    local bmb_src="benches/$category/$name/bmb/main.bmb"
    local c_src="benches/$category/$name/c/main.c"
    local rust_src="benches/$category/$name/rust/main.rs"
    local bmb_exe="$RESULTS_DIR/${name}_bmb.exe"
    local c_exe="$RESULTS_DIR/${name}_c.exe"
    local rust_exe="$RESULTS_DIR/${name}_rust.exe"

    printf "%-20s " "$name"

    # Build and run BMB
    if [ -f "$bmb_src" ]; then
        if $BMB build "$bmb_src" -o "$bmb_exe" > /dev/null 2>&1; then
            bmb_time=$(run_benchmark "$bmb_exe")
            printf "BMB:%5dms " "$bmb_time"
        else
            printf "BMB:${RED}FAIL${NC}   "
            bmb_time="FAIL"
        fi
    else
        printf "BMB:${YELLOW}N/A${NC}    "
        bmb_time="N/A"
    fi

    # Build and run C (Clang)
    if [ -f "$c_src" ]; then
        if clang -O3 -march=native -o "$c_exe" "$c_src" -lm 2>/dev/null; then
            c_time=$(run_benchmark "$c_exe")
            printf "C:%5dms " "$c_time"
        else
            printf "C:${RED}FAIL${NC}   "
            c_time="FAIL"
        fi
    else
        printf "C:${YELLOW}N/A${NC}    "
        c_time="N/A"
    fi

    # Build and run Rust
    if [ -f "$rust_src" ]; then
        if rustc -O -o "$rust_exe" "$rust_src" 2>/dev/null; then
            rust_time=$(run_benchmark "$rust_exe")
            printf "Rust:%5dms " "$rust_time"
        else
            printf "Rust:${RED}FAIL${NC}   "
            rust_time="FAIL"
        fi
    else
        printf "Rust:${YELLOW}N/A${NC}    "
        rust_time="N/A"
    fi

    # Calculate ratios
    if [[ "$bmb_time" =~ ^[0-9]+$ ]] && [[ "$c_time" =~ ^[0-9]+$ ]] && [ "$c_time" -gt 0 ]; then
        ratio_c=$(awk "BEGIN {printf \"%.2f\", $bmb_time / $c_time}")
        ratio_c_ok=$(awk "BEGIN {print ($bmb_time / $c_time <= 1.1) ? 1 : 0}")
        if [ "$ratio_c_ok" -eq 1 ]; then
            printf "${GREEN}vs C:${ratio_c}x${NC} "
        else
            printf "${YELLOW}vs C:${ratio_c}x${NC} "
        fi
    fi

    if [[ "$bmb_time" =~ ^[0-9]+$ ]] && [[ "$rust_time" =~ ^[0-9]+$ ]] && [ "$rust_time" -gt 0 ]; then
        ratio_rust=$(awk "BEGIN {printf \"%.2f\", $bmb_time / $rust_time}")
        ratio_rust_ok=$(awk "BEGIN {print ($bmb_time / $rust_time <= 1.1) ? 1 : 0}")
        if [ "$ratio_rust_ok" -eq 1 ]; then
            printf "${GREEN}vs Rust:${ratio_rust}x${NC}"
        else
            printf "${YELLOW}vs Rust:${ratio_rust}x${NC}"
        fi
    fi

    echo ""
}

echo "=== COMPUTE BENCHMARKS ==="
echo ""

for dir in benches/compute/*/; do
    name=$(basename "$dir")
    benchmark "$name" "compute"
done

echo ""
echo "=== REAL-WORLD BENCHMARKS ==="
echo ""

for dir in benches/real_world/*/; do
    name=$(basename "$dir")
    benchmark "$name" "real_world"
done

echo ""
echo "Done! Results in $RESULTS_DIR/"
